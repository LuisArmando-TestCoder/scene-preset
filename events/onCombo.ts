...
export default function onCombo(combo: string, callback: Function, isSorted = true) {
    ...
    keys.forEach(key => {
        onKey(key).start(() => {
            ...
            const havingUnsortedCombo = getSortedLetters(keysState.queue) === normalizedCombo
